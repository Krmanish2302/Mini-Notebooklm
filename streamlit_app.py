"""
streamlit_app.py — Mini NotebookLM · Streamlit UI (v3)

What changed vs v2
------------------
* Follow-up suggestion buttons are now CLICKABLE — clicking one writes
  the question into st.session_state.pending_query which is picked up
  by the chat input on the next rerun.
...
_DEFAULTS: Dict[str, Any] = {
    ...
    "pending_query":  "",   # FIX #5: clicked follow-up pre-fills chat input
}
...
with st.sidebar:
    ...
    if st.button("💾 Apply Config", use_container_width=True):
        ...
        st.success("Config updated ✓") if (r and r.ok) else st.error(r.text if r else "Backend unreachable")
                ...
                with st.status(f"Ingesting {pdf_file.name} …", expanded=True) as status:
                    ...
                    try:
                        ...
                        if r and r.ok:
                            ...
                            bar.progress(100); status.update(label="✅ Ingested!", state="complete"); st.rerun()
                        else:
                            status.update(label="❌ Failed", state="error"); st.error(r.text if r else "Backend unreachable")
                    except Exception as e:
                        status.update(label="❌ Error", state="error"); st.error(str(e))
    ...
    for s in sources:
        ...
        if col_del.button("🗑", key=f"del_{sid}", help=f"Delete {name}"):
            ...
            if r and r.ok:
                st.session_state.active_sources.pop(sid, None); st.rerun()
            else:
...
# FIX #5: if a follow-up button was clicked, pre-fill the chat input
...
if _pending:
    # Clear pending so it doesn't re-fire on next rerun
...
if prompt:
    ...
    with st.chat_message("assistant"):
        ...
        try:
            ...
            # FIX #5: clickable follow-up buttons ─────────────────────────────
